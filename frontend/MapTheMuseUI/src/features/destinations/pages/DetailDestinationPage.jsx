...
import FavouriteDestinationButton from "../../favourites/components/FavourtieDestinationButton";
...
export default function DetailDestinationPage() {
    ...
    const [{ items: media, loading: mediaLoading, error: mediaError }] = useDestinationMedia(numId);
    ...
    useEffect(() => {
        ...
        // include deps so it updates correctly when context changes
    }, [numId, byId, getById]);

    const movies = useMemo(
        () => (media ?? []).filter((m) => (m.type || "").toLowerCase() === "movie"),
        [media]
    );
    const tv = useMemo(
        () => (media ?? []).filter((m) => (m.type || "").toLowerCase() === "tv"),
        [media]
    );
    ...
    if (error || mediaError) {
        return (
            <Container sx={{ py: 6 }}>
                <Typography color="error">Couldn’t load destination or media.</Typography>
            </Container>
        );
    }
    if (!destination) {
        ...
        <Box sx={{ pb: 6 }}>
            ...
            >
                ...
                <Box
                    sx={{
                        position: "absolute",
                        inset: 0,
                        background: "linear-gradient(0deg, rgba(132,132,132,0.35), rgba(246,246,246,0.35))",
                    }}
                />
                {/* centred title */}
                <Box
                ...
                >
                    ...
                    <Stack direction="row" spacing={1} sx={{ ml: "auto" }}>
                        <FavouriteDestinationButton destinationId={numId} light />
                        <Tooltip title="Share">
            ...
            <Container sx={{ position: "sticky", top: 0, zIndex: 8, py: 1, bgcolor: "background.default" }}>
                <Stack direction="row" spacing={1.5} sx={{ overflowX: "auto" }}>
                    ...
                    {!!movies.length && <Button href="#films" size="small" variant="text">Films</Button>}
                    {!!tv.length && <Button href="#tv" size="small" variant="text">TV</Button>}
                </Stack>
                <Divider sx={{ mt: 1 }} />
            </Container>
            ...
            <Container sx={{ mt: 3 }}>
                <Grid container spacing={5}>
                    ...
                    <Grid size={12}>
                        ...
                        <Section id="soundtrack" title="Soundtrack" subtitle="Plug in and set the mood for your trip">
                            <Paper elevation={0} sx={{ p: 1.5, borderRadius: 3, border: (t) => `1px solid ${t.palette.divider}` }}>
                        ...
                        {!!movies.length && (
                            <>
                                <Divider sx={{ my: 3 }} />
                                <Section id="films" title="Films" subtitle="Stories shot in or set around this destination">
                        ...
                        {!!tv.length && (
                            <>
                                <Divider sx={{ my: 3 }} />
                                <Section id="tv" title="TV" subtitle="Series that bring the place to life">
                                    <MediaRail items={tv} />
                                </Section>
                            </>
                        )}
